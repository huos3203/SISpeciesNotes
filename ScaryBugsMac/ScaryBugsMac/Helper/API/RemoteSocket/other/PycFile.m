        ...
        haveOutStru:(NSInteger *)haveOutLine
{
    ...
    int fileheadoffset = (fileSize.longValue > structsize)? (fileSize.longValue - structsize):0;
    if(fileheadoffset == 0)
    {
        NSLog(@"nothing in in");
        return FALSE;
...
-(BOOL)makePycFile
{
    ...
    //补齐文件长度
    int fileToAdd = [fileSizeOriginal longValue];
    int a = (fileToAdd%16);
    if (a != 0) {
...
//        }
    ...
    int j1=[arry[0] longValue];
    int j2=[arry[1] longValue];
...
-(int)isOutLineFileCanSee:(NSString*)fileName remainDay:(NSInteger*)theRemainDay remainYear:(NSInteger *)theRemainYear
{
    ...
    if (fileAttributes != nil) {
        ...
        fileSize = [fileAttributes objectForKey:NSFileSize];
        NSLog(@"离线文件size=%d", fileSize.longValue);
    }
